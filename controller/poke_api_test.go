...
func TestPokemonFromApiSuccess(t *testing.T) {
	c := require.New(t)
	pokemon, err := GetPokemonFromPokeApi("bulbasaur")
	c.NoError(err)

	body, err := ioutil.ReadFile("sample/poke_apiResponse.json")
	c.NoError(err)
	var expeted models.PokeApiPokemonResponse
	err = json.Unmarshal([]byte(body), &expeted)
	c.NoError(err)

	c.Equal(expeted, pokemon)
}
...
func TestGetPokemonFromPokeApiSuccessWithMocks(t *testing.T) {
	...
	body, err := ioutil.ReadFile("sample/poke_api_response.json")
	c.NoError(err)
...
func TestGetPokemon(t *testing.T) {
	...
	expectedBodyResponse, err := ioutil.ReadFile("sample/api_response.json")
	c.NoError(err)
